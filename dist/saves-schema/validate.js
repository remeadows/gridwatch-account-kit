...
function check(schema, value, path, depth, budget) {
    ...
    switch (schema.type) {
        ...
        case "number": {
            ...
            if (value < min)
                return fail(min === schema.min ? `${path}: below minimum ${min}` : `${path}: out of range (below minimum ${min})`);
            if (value > max)
                return fail(max === schema.max ? `${path}: above maximum ${max}` : `${path}: out of range (above maximum ${max})`);
            return { ok: true };
        }
        case "array": {
