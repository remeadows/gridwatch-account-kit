...
export function validateReturnPath(raw: string | null | undefined, nexusOrigin: string = NEXUS_ORIGIN): string {
  if (!raw || RAW_FORBIDDEN.test(raw)) return "/";
  // Check dot segments on the RAW path before new URL() normalizes them away.
  if (SEGMENT_FORBIDDEN.test(raw.split(/[?#]/)[0])) return "/";
  let url: URL;
  try { url = new URL(raw, nexusOrigin); } catch { return "/"; }
  if (url.origin !== new URL(nexusOrigin).origin) return "/";
  if (raw.startsWith("//")) return "/";
  const path = url.pathname;
  if (SEGMENT_FORBIDDEN.test(path)) return "/";
  if (path === "/") return "/";
...
export function signInUrl(returnPath: string, nexusOrigin: string = NEXUS_ORIGIN): string {
