...
export function readCarryMessage(data) {
    if (!isRecord(data) || data.gw !== "carry" || data.v !== 1)
        return null;
    const id = data.id;
    ...
    switch (data.type) {
        ...
        case "offer": {
            ...
            if (typeof gameSlug !== "string" || typeof schemaVersion !== "number" || !isRecord(slots) || typeof exportedAt !== "string")
                return null;
            return { gw: "carry", v: 1, type: "offer", id, gameSlug, schemaVersion, slots: slots, exportedAt };
        }
        case "result": {
            const status = data.status;
            if (status !== "accepted" && status !== "declined" && status !== "rejected")
                return null;
            const message = { gw: "carry", v: 1, type: "result", id, status };
            if (typeof data.detail === "string")
                message.detail = data.detail.slice(0, 200);
            return message;
        }
        default:
            return null;
...
export function assertCarryOrigins(origins) {
    for (const origin of origins) {
        ...
        if (!ok)
            throw new Error(`[account-kit] carryFrom: "${origin}" is not an exact https origin (or a loopback http origin)`);
    }
}
/** Receiver-side check of an offer against this game's kit config (spec §6.2). */
export function checkOffer(offer, game) {
    if (offer.gameSlug !== game.gameSlug)
        return { ok: false, detail: `wrong game "${offer.gameSlug}"` };
    if (offer.schemaVersion !== game.schemaVersion)
        return { ok: false, detail: `wrong schemaVersion ${offer.schemaVersion}` };
    const slots = Object.keys(offer.slots);
    if (slots.length === 0)
        return { ok: false, detail: "no slots" };
    for (const slot of slots) {
        if (!game.slots.includes(slot))
            return { ok: false, detail: `unknown slot "${slot}"` };
        const payload = offer.slots[slot];
        const valid = validatePayload(game.gameSlug, game.schemaVersion, slot, payload);
        if (!valid.ok)
            return { ok: false, detail: `${slot}: ${valid.detail}` };
        let bytes;
        try {
        ...
        if (bytes > MAX_BODY_BYTES)
            return { ok: false, detail: `${slot}: payload too large` };
    }
