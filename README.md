...
- **`./saves-schema`** — the `/api/saves` wire contract shared by the Nexus worker and the kit client: `SAVE_GAMES`, `resolveSaveGame`, `payloadSchemas`, `validatePayload`, `validateAgainst`, `findDeniedKey`, `DENYLIST`, `canonicalJson`, `hasLoneSurrogate`, `requestHash`, `sha256Hex`, `MAX_BODY_BYTES`, `UUID_RE`, `SLOT_RE`, `ALIAS_RE`, and the wire types. DOM-free, dependency-free.
  ...
  Contract note (restated in v0.2.6, enforced since the validator's first release): an `integer` schema value must be a *safe* integer (`Number.isSafeInteger`, i.e. within ±(2^53 − 1)) whether or not the schema declares a `min`/`max` — so a value like `1e308`, which is tiny on the wire but ~309 characters once stored as Postgres `jsonb` text, is rejected by the client and the worker alike instead of overrunning the database's byte backstop. `requestHash` hashes every payload `validatePayload` accepts, up to the full depth limit (fixed in v0.2.6: a payload at exactly depth 32 used to validate and then fail to hash).
