...
**Several tabs, one account.** Every tab of the origin shares the per-user `{ revision, dirty }` record, so a cloud `GET` can come back *older* than a revision another tab has confirmed since it was sent. Since v0.2.6 the kit never lets that roll anything back: a record's revision never decreases (an older confirmation is ignored; an older "dirty" still marks the record dirty at its current revision), and `reconcile` — and the "Use cloud" answer at either conflict prompt — never decides on or hands back a row older than the record. It re-loads the row once; if the cloud still answers with the older row, the call resolves `{ status: "error", error: { code: "network", message: "cloud row is older than a revision this browser already confirmed; try again" } }` without changing the record, the ownership record or anything remembered for a re-flush. Treat it like any other `network` error: retry the call.
